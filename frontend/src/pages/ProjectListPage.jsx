      ...
      {loading ? (
        ...
        <div className="text-center py-20 bg-white rounded-xl border border-dashed border-gray-200">
          ...
          <p className="text-sm text-gray-400 mb-6 max-w-sm mx-auto">
            Create your first project to start screening molecules against a target protein.
          </p>
          <button
            onClick={() => { setShowCreate(true); setCreateError('') }}
            className="inline-flex items-center gap-2 px-5 py-2.5 bg-[#22c55e] text-white rounded-lg font-medium hover:bg-[#16a34a] transition-colors"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
            </svg>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {projects.map(p => (
            <Link
              key={p.id}
              to={`/project/${p.id}`}
              className="bg-white rounded-xl border border-gray-100 shadow-sm hover:shadow-md hover:border-[#1e3a5f]/20 transition-all group block overflow-hidden"
            >
              <div className="flex">
                {/* Status bar */}
                <div className={`w-1 shrink-0 ${
                  (p.completed_count || 0) > 0 ? 'bg-[#22c55e]' :
                  (p.running_count || 0) > 0 ? 'bg-blue-500' :
                  'bg-gray-200'
                }`} />
                <div className="flex-1 p-5">
                  ...
                  {/* Target badge */}
                  {p.uniprot_id && (
                    <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-blue-50 text-blue-700 text-xs font-mono rounded-md mb-2">
